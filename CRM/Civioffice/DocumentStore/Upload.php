...
class CRM_Civioffice_DocumentStore_Upload extends CRM_Civioffice_DocumentStore {
  ...
  public const UPLOAD_PUBLIC_ENABLED_SETTINGS_KEY = 'civioffice_store_upload_public';

  public const UPLOAD_PRIVATE_ENABLED_SETTINGS_KEY = 'civioffice_store_upload_private';
  ...
  public function isReady() : bool {
    $enabled = (bool) Civi::settings()->get(
      $this->common ? self::UPLOAD_PUBLIC_ENABLED_SETTINGS_KEY : self::UPLOAD_PRIVATE_ENABLED_SETTINGS_KEY
    );

    return $enabled && file_exists($this->folder_name) && is_dir($this->folder_name);
  }
  ...
  public function getDocumentByURI(string $uri): ?CRM_Civioffice_Document {
    ...
    if (substr($uri, 0, 7) == 'local::') {
      ...
      $this->base_folder = $this->folder_name;
